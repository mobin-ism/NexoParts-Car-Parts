...
class MarcaSelector: NSObject {
    ...
    let cellId = "marcaCell"
    var agregarArticuloObj = AgregarArticulo()
    
    override init() {
        super.init()
        tableView.register(MarcaCell.self, forCellReuseIdentifier: cellId)
    }
...
extension MarcaSelector: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        if let cell = tableView.dequeueReusableCell(withIdentifier: cellId, for: indexPath) as? MarcaCell {
            if let data = selectorData as? [Marca] {
                cell.titleText = "\(data[indexPath.row].nombreMarca)"
            }
            if let data = selectorData as? [Modelo]{
                cell.titleText = "\(data[indexPath.row].nombreModelo)"
            }
            return cell
        } else {
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + 0.1, execute: {
            if let data = self.selectorData as? [Marca] {
                self.agregarArticuloObj.changeMarcaSelectorTitle(withString: "\(data[indexPath.row].nombreMarca)    ▼")
                self.agregarArticuloObj.selectedMarca = data[indexPath.row].idMarca
            }
            if let data = self.selectorData as? [Modelo] {
                self.agregarArticuloObj.changeModeloSelectorTitle(withString: "\(data[indexPath.row].nombreModelo)    ▼")
                self.agregarArticuloObj.selectedModelo = data[indexPath.row].idModel
            }
        })
    }
